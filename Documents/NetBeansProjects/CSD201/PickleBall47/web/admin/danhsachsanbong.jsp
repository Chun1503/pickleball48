<%-- 
    Document   : danhsachsanbong
    Created on : Mar 14, 2025, 6:55:22 PM
    Author     : Minh Trung
--%>
    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
        ...
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                <a href="index1.jsp" class="navbar-brand mx-4 mb-3">
                    <h3 class="text-primary">PickleBall47</h3>
                </a>
                <div class="navbar-nav w-100">
					...
					<div class="nav-item">
                        ...
                        <div class="nav-item">
                            <a href="Dashboard" class="nav-link" ><i class="fa fa-laptop me-2"></i>Dashboard</a>                            
                        </div>
                </div>
            </nav>
        </div>
        ...
        <div class="content">
                ...
                <table class="table" style="width:100%">
                    ...
                    <tbody>
                        <c:forEach var="list" items="${requestScope.listffandffs}">
                            <tr id="${list[0].pickleBallField.IDPickleBallField}">
                                <td>${list[0].pickleBallField.name}</td>
                                <td id="${list[0].pickleBallField.image}">
                                    <img src="../img_nhat/${list[0].pickleBallField.image}" alt="Hình ảnh sân" style="width: 100%; height: 80px" data-bs-toggle="modal" data-bs-target="#myModal" onclick="getImageId(this)">
                                </td>
                                <td>Sân ${list[0].pickleBallField.typeofPickleBallField} người/Đội</td>
                                <td>${list[0].pickleBallField.price} VNĐ</td>
                                <td>
                                ...
                                <td>
                                    <button class="btn btn-danger" type="button" onclick="editSanBong(${list[0].pickleBallField.IDPickleBallField})">Chỉnh sửa</button>
                                    <button class="btn btn-dark" type="button" onclick="deleteSanBong(${list[0].pickleBallField.IDPickleBallField}, this)">Xóa sân bóng</button>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
            ...
            <script>
                ...
                var pickleBallFieldId1;
                function editSanBong(pickleBallFieldId) {
                    pickleBallFieldId1 = pickleBallFieldId;
                    // Chuyển hướng đến trang /admin/updatesanbong với thông tin của list
                    window.location.href = 'updatesanbong?pickleBallFieldId=' + pickleBallFieldId;

                }
                function deleteSanBong(pickleBallFieldId) {
                    // Hiển thị modal xác nhận
                    $('#confirmDeleteModal').modal('show');

                    // Lưu ID sân bóng vào input hidden
                    $('#deletePickleBallFieldId').val(pickleBallFieldId);
                }

                function confirmDelete() {
                    // Lấy ID sân bóng từ input hidden
                    var pickleBallFieldId = $('#deletePickleBallFieldId').val();
                    ...
                    $.ajax({
                        type: "POST",
                        url: "http://localhost:8080/PickleBall47/admin/deletesanbong",
                       data: {pickleBallFieldId: pickleBallFieldId}, 
                        success: function (response) {
                            // Xử lý phản hồi từ máy chủ nếu cần
                            console.log(response);

                            // Xóa thẻ <tr> khi Ajax thành công
                            $('tr#' + pickleBallFieldId).remove();
                        },
                        error: function (error) {
                            // Xử lý lỗi nếu có
                            console.error("Error deleting pickleBall field:", error);
                        }
                    });
                }
                function addSanmoi() {
            ...
            <div class="modal fade" id="confirmDeleteModal" tabindex="-1" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content" ">
                        ...
                        <input type="hidden" id="deletePickleBallFieldId">
                    </div>
                </div>
            </div>
                ...
                <div class="bg-light rounded-top p-4">
                    <div class="row">
                        <div class="col-12 col-sm-6 text-center text-sm-start">
                            <a href="index1.jsp">PickleBall47</a>, All Right Reserved. 
                        </div>                        
                    </div>
                </div>
            </div>
