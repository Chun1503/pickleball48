<%-- 
    Document   : doanhthu
    Created on : Mar 15, 2025, 3:48:50 PM
    Author     : Minh Trung
--%>
...
<html lang="en">
    ...
    <tr id="${bill.IDBill}">
        ...
        <td>${bill.invoice}</td>
        <c:choose>
            <c:when test="${bill.registeredPickleBallField.status == 2}">
                <td>Cọc Sân</td>
            </c:when>
            <c:when test="${bill.registeredPickleBallField.status == 1}">
        ...
        <td><fmt:formatNumber value="${bill.totalPrice}" pattern="#,##0" /> VNĐ</td>
        <td>
            <c:choose>
                <c:when test="${bill.registeredPickleBallField.status == 5}">
                    Đã hoàn tiền
                </c:when>
                <c:otherwise>
                    Đã thanh toán
