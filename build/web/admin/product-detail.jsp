    ...
    <main class="main-content container" style="width: 90%">
          ...
          <div>
                <form class="modal-form" id="modal-add-form" method="post" 
                      action="${pageContext.request.contextPath}/admin/products/${product.product_id}" 
                      enctype="multipart/form-data"> 
                    <label for="modal-add-name">
                    <span>Tên sản phẩm</span>
                    <input type="text" id="modal-add-name" name="name" value="${product.name}" />
                ...
                <label for="modal-add-title">
                    <span>Giá</span>
                    <input type="number" id="modal-add-title" name="price" value="${product.price}" />
                    <p class="modal-error" id="modal-add-title-error"></p>
                </label>

                <label for="modal-add-vendor">
                    <span>Số lượng</span>
                    <input type="number" id="modal-add-vendor" name="qty" value="${product.stock_quantity}" />
                ...
                <label for="modal-add-image">
                    ...
                    <c:if test="${not empty product.image}">
                        <img src="${pageContext.request.contextPath}/${product.image}" width="100" />
                    </c:if>
                </label>

                <label for="modal-add-desc" class="modal-desc-label">
                    <span>Mô tả</span>
                    <textarea id="modal-add-desc" name="desc" rows="5" maxlength="400">${product.description}</textarea>
                ...
                <div class="modal-buttons">
                    <button type="button" class="cancel" id="add-close-button">Hủy</button>
